...
void(*LZMA_Progress)(float) = NULL;
void(*LZMA_ListFile)(const char*, int) = NULL;
int(*LZMA_FilterFile)(const char*, int) = NULL;
    ...
    if (res == SZ_OK)
    {
      ...
      */
      ...
      for (i = 0; i < db.db.NumFiles; i++)
      {
        ...
        if (db.db.NumFiles>1 && LZMA_Progress!=NULL)
            LZMA_Progress((float)i/((float)db.db.NumFiles-1.0f));
        ...
        if (LZMA_ListFile!=NULL || LZMA_FilterFile!=NULL)
        {
            CBuf buf;
            SRes res;
            Buf_Init(&buf);
            ...
            if (res == SZ_OK)
            {
                if (LZMA_ListFile!=NULL)
                    LZMA_ListFile((const char *)buf.data, f->IsDir);
                if (LZMA_FilterFile!=NULL)
                    filter = LZMA_FilterFile((const char *)buf.data, f->IsDir);
            }
            Buf_Free(&buf, &g_Alloc);
        }

        if (listCommand || filter==0)
            continue;
