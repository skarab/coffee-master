...
void CustomPlayerController::SetVelocity (dFloat forwardSpeed, dFloat sideSpeed, dFloat jumpSpeed, dFloat heading)
{
	m_heading = heading;
...
{
    NewtonCollision* c1 = NewtonBodyGetCollision(body);
    if(NewtonCollisionIsTriggerVolume(c1))
        return 0;
...
unsigned CustomPlayerController::TriggerCastPrefilter(const NewtonBody* body, const NewtonCollision* collision, void* userData)
{
    CastFilterData* filterData;
    filterData = (CastFilterData*) userData;
	for (int i =0; i < filterData->m_count;i ++){
...
void CustomPlayerController::PlayerOnLand (dFloat timestep, int threadIndex)
{
    ...
    _ASSERTE (dgCheckVector ((velocity)));
    velocity -= m_gravity.Scale (timestep);
	velocity = CalculateVelocity (velocity, timestep, upDir, m_stairHeight, threadIndex);
    _ASSERTE (dgCheckVector ((velocity)));
...
void CustomPlayerController::KinematicMotion (dFloat timestep, int threadIndex)
{
    ...
    turnAngle = turnAngle>-1.0f?turnAngle:-1.0f;
    turnAngle = turnAngle<1.0f?turnAngle:1.0f;
    ...
    if (OnCollide!=NULL)
    {
	    ...
	    CastFilterData castFilterData (m_body0);
	    NewtonWorldConvexCastReturnInfo info[MAX_CONTACTS];
        dMatrix mat(bodyMatrix);
        dVector target(bodyMatrix.m_posit);
        /*mat.m_posit.m_x += upDir.m_x-frontDir.m_x*4.0f;
        mat.m_posit.m_y += upDir.m_y;
        mat.m_posit.m_z += upDir.m_z-frontDir.m_z*4.0f;*/
        target.m_x += frontDir.m_x*4.0f;
        target.m_y += frontDir.m_y*4.0f;
        target.m_z += frontDir.m_z*4.0f; 
        contacts = NewtonWorldConvexCast (m_world, &mat[0][0], &target.m_x,
            m_bodySensorShape, &hitParam, &castFilterData, TriggerCastPrefilter,
            info, sizeof(info)/sizeof(info[0]), threadIndex);
        if (contacts)
        {
            for (int i=0 ; i<contacts ; ++i)
                (*OnCollide)(NULL, m_body0, info[i].m_hitBody, 0);
        }
    }
