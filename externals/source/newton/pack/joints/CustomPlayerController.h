...
class JOINTLIBRARY_API CustomPlayerController: public NewtonCustomJoint
{
    ...
    bool IsJumping() const { return m_playerState==m_onFreeFall; }
    
	dFloat GetMaxSlope () const;
	void SetMaxSlope (dFloat maxSlopeAngleIndRadian);
	void SetVelocity (dFloat forwardSpeed, dFloat sideSpeed, dFloat jumpSpeed, dFloat heading);
...
*/	
    ...
    static unsigned TriggerCastPrefilter(const NewtonBody* body, const NewtonCollision* collision, void* userData);

public:
    int (*OnCollide)(const NewtonMaterial*, const NewtonBody*, const NewtonBody*, int);
