...
static int StrSubst (const char * in, const char * pat, const char * rep, const size_t maxLen, char * out) {
  ...
  while ((si < inLen) && (di < maxLen)) {
    if (patLen <= (inLen - si)) {
       if (strnicmp (& in [si], pat, patLen) == 0) {
          if (repLen > (maxLen - di)) {
             repLen = maxLen - di;
             truncated = 1;
          }
...
static int ProcessMetrics (TokenReaderT * tr, const uint fftSize, const uint truncSize, HrirDataT * hData) {
  ...
  while (! (hasRate && hasPoints && hasAzimuths && hasRadius && hasDistance)) {
    ...
    if (stricmp (ident, "rate") == 0) {
       ...
       hasRate = 1;
    } else if (stricmp (ident, "points") == 0) {
       if (hasPoints) {
       ...
       hasPoints = 1;
    } else if (stricmp (ident, "azimuths") == 0) {
       if (hasAzimuths) {
       ...
       hasAzimuths = 1;
    } else if (stricmp (ident, "radius") == 0) {
       if (hasRadius) {
       ...
       hData -> mRadius = fpVal;
       hasRadius = 1;
    } else if (stricmp (ident, "distance") == 0) {
       if (hasDistance) {
...
static SourceFormatT MatchSourceFormat (const char * ident) {
  if (stricmp (ident, "wave") == 0)
     return (SF_WAVE);
  else if (stricmp (ident, "bin_le") == 0)
     return (SF_BIN_LE);
  else if (stricmp (ident, "bin_be") == 0)
     return (SF_BIN_BE);
  else if (stricmp (ident, "ascii") == 0)
     return (SF_ASCII);
  return (SF_NONE);
}
...
static ElementTypeT MatchElementType (const char * ident) {
  if (stricmp (ident, "int") == 0)
     return (ET_INT);
  else if (stricmp (ident, "fp") == 0)
     return (ET_FP);
  return (ET_NONE);
}
