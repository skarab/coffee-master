...
static void alc_init(void)
{
    ...
    if(str && (stricmp(str, "true") == 0 || strtol(str, NULL, 0) == 1))
        ConeScale *= 0.5f;
...
static void alc_initconfig(void)
{
    ...
    {
        char buf[1024] = "";
        int len = sprintf(buf, "%s", BackendList[0].name);
        for(i = 1;BackendList[i].name;i++)
            len += sprintf(buf+len, ", %s", BackendList[i].name);
    ...
    if(ConfigValueStr(NULL, "disable-cpu-exts", &str))
    {
        ...
        else
        {
            ...
            do {
                ...
                if(strnicmp(str, "sse", len) == 0)
                    capfilter &= ~CPU_CAP_SSE;
                else if(strnicmp(str, "neon", len) == 0)
                    capfilter &= ~CPU_CAP_NEON;
                else
    ...
    {
        if(stricmp(str, "point") == 0 || stricmp(str, "none") == 0)
            DefaultResampler = PointResampler;
        else if(stricmp(str, "linear") == 0)
...
static void alcSetError(ALCdevice *device, ALCenum errorCode)
{
    if(TrapALCError)
    {
#ifndef COFFEE_COMPILER_MSVC2005
#ifdef _WIN32
...
ALC_API ALCdevice* ALC_APIENTRY alcOpenDevice(const ALCchar *deviceName)
{
    ...
    if(deviceName && (!deviceName[0] || stricmp(deviceName, alcDefaultName) == 0 || stricmp(deviceName, "openal-soft") == 0))
        deviceName = NULL;
    ...
    if(ConfigValueStr(NULL, "channels", &fmt))
    {
        ...
        {
            if(stricmp(chanlist[i].name, fmt) == 0)
            {
    ...
    if(ConfigValueStr(NULL, "sample-type", &fmt))
    {
        ...
        {
            if(stricmp(typelist[i].name, fmt) == 0)
            {
...
#define DEVICE_FORMAT_REQUEST (DEVICE_CHANNELS_REQUEST|DEVICE_SAMPLE_TYPE_REQUEST)
       ...
       ConfigValueStr(NULL, "format", &fmt))
    {
        ...
        {
            if(stricmp(fmt, formats[i].name) == 0)
            {
