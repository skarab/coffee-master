...
uniform sampler2D hotspot_texture;
...
void main()
{
    ...
    float illumination = light_intensity*texture2D(fallof_texture, vec2(distance/light_radius, 0.0)).x;
    vec4 light_space_position = camera_to_light_matrix * vec4(position, 1.0);
    light_space_position.z = min(light_space_position.z, 0.0);
    float apex = 50.0/(light_space_position.z*light_apex);    
    vec2 apex_position = light_space_position.xy*apex;
    apex_position *= 0.5;
    apex_position += 0.5;    
    illumination *= texture2D(hotspot_texture, apex_position).x;
