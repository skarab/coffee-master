// -------------------------------------------------------------------------------------------------
// Fake soft shadows filter - horizontal pass
// -------------------------------------------------------------------------------------------------
...
varying vec3 view_vector;
    ...
    uniform usampler2DRect stencil_texture;
    
    float ComputeSoftShadows(float distance, float depth, vec3 normal)
    {
        float radius = distance*distance*light_softness/(frame_size.x*depth/6.0);
        
        float stencil = 0.01;
        float count = stencil;
        for (float i=-4.0 ; i<=4.0 ; i+=0.5)
        {
            vec2 t = vec2(uv.x+i*radius, uv.y);
            vec3 n = UnpackNormal(texture2DRect(normal_texture, t).xyz);
            float coef = (5.0-abs(i))*float(dot(n, normal)>0.9);
            stencil += coef*float(texture2DRect(stencil_texture, t).x==unsigned int(128));
            count += coef;
        }
