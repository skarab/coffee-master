...
<VisualStudioProject
	...
	for platformName in ivalues(Config.Platforms) do
		for configName in ivalues(Config.Configurations) do
            ...
            jamCommandLine = coffee_root_path .. 'tools/python/python.exe ' .. coffee_root_path .. 'tools/scripts/build.py workspace ' .. platformName .. ' ' .. configName
            
            if self.Options.vs2005 then
                jamCommandLine = jamCommandLine .. ' vs2005'
            elseif self.Options.vs2008 then
                jamCommandLine = jamCommandLine .. ' vs2008'
            end
		...
		<Configuration
			...
			ConfigurationType="0"
			BuildLogFile="$(destinationRootPath:gsub('\\', '/'))$(Platform)-vs2003-$(Config)/BuildLog.htm">
			<Tool
				Name="VCNMakeTool"
			...
			IntermediateDirectory="$$(ConfigurationName)"
			ConfigurationType="0"
			BuildLogFile="$(destinationRootPath:gsub('\\', '/'))$(Platform)-vs2005-$(Config)/BuildLog.htm"
			>
			<Tool
				Name="VCNMakeTool"
				BuildCommandLine="$(BuildCommandLine)"
			...
			IntermediateDirectory="$$(ConfigurationName)"
			ConfigurationType="0"
			BuildLogFile="$(destinationRootPath:gsub('\\', '/'))$(Platform)-vs2008-$(Config)/BuildLog.htm"
			>
			<Tool
				Name="VCNMakeTool"
				BuildCommandLine="$(BuildCommandLine)"
